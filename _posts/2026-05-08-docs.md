...
If you develop from the monorepo checkout, use `templates/soonex_i18n`. The default is **`{:corex, "~> 0.1.0-beta.4"}`** from Hex; to build against a sibling **`corex`** repo, switch **`mix.exs`** to **`{:corex, path: "../../corex"}`** and run **`mix deps.get`** so **`deps/corex`** matches that checkout (the **`corex/hooks`** and **`corex/select`** style imports in **`assets/js/site.js`** stay the same).
