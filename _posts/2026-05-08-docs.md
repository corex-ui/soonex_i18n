...
If you develop from the monorepo checkout, use `templates/soonex_i18n`. The default is **`{:corex, "~> 0.1.0-beta.3"}`** from Hex; to build against a sibling **`corex`** repo, switch **`mix.exs`** to **`{:corex, path: "../../corex"}`** and run **`mix deps.get`** so **`deps/corex`** matches that checkout (the **`corex/hooks`** and **`corex/select`** style imports in **`assets/js/site.js`** stay the same).
...
4. `mix assets.build`
5. Either `mix soonex_i18n.tableau_build` for a production tree under `_site/`, or `mix tableau.server` for the dev server with reload.
...
- Run `mix setup`
- Build with `mix soonex_i18n.tableau_build` or serve with `mix tableau.server`
...
Site-root paths resolve after `mix soonex_i18n.tableau_build` (static files live under `_site/`):
...
Inline highlighter: `Mix.Task.run("soonex_i18n.tableau_build")` and `mix deps.get`.
...
> **Tip:** After `mix soonex_i18n.tableau_build`, open `_site/en/docs/index.html` to confirm exactly what shipped for this permalink.
